...
public class LoginController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String service = request.getParameter("login");
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (service.equals("login")) {
                ...
                if (!daoCustomers.isExistUser(username, password)) {
                    request.setAttribute("error", "Username or Password are incorrect");
                    dispath(request, response, "JSP/login.jsp");
                } else {
                    ResultSet rsOfUsername = daoCustomers.getCustomerByUsername(username);

                    Account account = null;
                    if (rsOfUsername.next()) {
                        account = new Account(rsOfUsername.getString(1), rsOfUsername.getString(2));
                    }
